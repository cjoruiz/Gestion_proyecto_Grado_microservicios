package co.edu.unicauca.messaging_microservice.infra.config;
...
public class SecurityConfig {
    ...
    // ✅ Decoder personalizado — NO valida issuer, solo timestamp
    @Bean
    @Primary
    public JwtDecoder jwtDecoder() {
        System.out.println("✅ Messaging: CUSTOM JwtDecoder con jwkSetUri: " + jwkSetUri);
    ...
    private Collection<GrantedAuthority> extractAuthoritiesFromJwt(Jwt jwt) {
        ...
        if (resourceAccess != null && resourceAccess.containsKey("sistema-desktop")) {
            ...
            if (roles != null && !roles.isEmpty()) {
                System.out.println("✅ Messaging: Roles encontrados: " + roles);
                return roles.stream()
                        .map(role -> new SimpleGrantedAuthority("ROLE_" + role))
                        .collect(Collectors.toList());
            }
        }
        System.out.println("❌ Messaging: No se encontraron roles");
